...
TEST_CASE("send_message") {
    ...
    SECTION("json"){
        ...
        REQUIRE(err > 0);
        char *buffer = new char[HEADER_SIZE + static_cast<int>(event.ByteSizeLong())];
        int len = full_read(fd2, *buffer, HEADER_SIZE + static_cast<int>(event.ByteSizeLong()));
        REQUIRE(static_cast<int>(len) == HEADER_SIZE + static_cast<int>(event.ByteSizeLong()));
        Header *header = new Header();
        deserialize(buffer, header);
        REQUIRE(header->size == (int32_t)event.ByteSizeLong());
        REQUIRE(header->id == 1);
        REQUIRE(header->type == DIFF_WRITE_ENABLE_EVENT);
        DiffWriteEnable recv_event;
        absl::string_view str(buffer + HEADER_SIZE, header->size);
        absl::Status status = google::protobuf::util::JsonStringToMessage(str, &recv_event);
        REQUIRE(status.ok());
