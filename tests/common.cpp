...
std::pair<int, int> testing_socket() {
    int fd[2];
...
TEST_CASE("send_message") {
    ...
    REQUIRE(err > 0);
    char *buffer = new char[HEADER_SIZE + (int)request.ByteSizeLong()];
    int len = full_read(fd2, buffer, HEADER_SIZE + (int)request.ByteSizeLong());
    REQUIRE((int)len == HEADER_SIZE + (int)request.ByteSizeLong());
    Header *header = deserialize(buffer);
