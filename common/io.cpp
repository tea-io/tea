...
template <typename T> int recv_handler_caller(char *recv_buffer, Header *header, int sock, int (*handler)(int sock, int id, T *response), bool json) {
    ...
    if (json) {
        ...
        if (!status.ok()) {
            log(ERROR, sock, "JsonStringToMessage failed: %s", status.message());
            return -1;
        }
    } else {
...
static int fs_handle_recv(int sock, Header *header, char *recv_buffer, fs_handlers &handlers, bool json) {
    int ret = -2;
    switch (header->type) {
...
static int event_handle_recv(int sock, Header *header, char *recv_buffer, event_handlers &handlers, bool json) {
    int ret = -2;
    switch (header->type) {
